...
However, the idea of the pattern came from below.
-----------------------------------------------
...
function draw() {
    ...
    savedCircles.sort((a, b) => { return a.currentX - b.currentX })
    for (let i = 0; i < savedCircles.length - 1; i++) {
        ...
        if (savedCircles && savedCircles.length > 10 && savedCircles[i]) {
            let j = (i + 1)
            for (; (savedCircles[i].currentX + lineLength) < savedCircles[j].currentX; j++) {
                let hypot = Math.hypot(Math.abs(savedCircles[i].currentX - savedCircles[j].currentX), Math.abs(savedCircles[i].currentY - savedCircles[j].currentY))
                if (hypot < lineLength) {
